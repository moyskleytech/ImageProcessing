...
{
    public unsafe partial class PixelImage
    {
        public PixelImage Resize(int width , int height , ScalingMode mode = ScalingMode.Auto)
        {
            return Rescale(width , height , mode);
        }
        public PixelImage Rescale(int width , int height , ScalingMode mode = ScalingMode.Auto)
        {
            if ( width <= 0 )
            ...
            PixelImage destination = new PixelImage(width,height);
        ...
        private void RescaleAverage(PixelImage destination , bool interpolate = false)
        {
            ...
            for ( var x = 0; x < destination.width; x++ )
            {
                for ( var y = 0; y < destination.height; y++ )
                {
                    ...
                    else
                    {
                        ...
                        if ( acount > 0 )
                        {
                            destinationpx.R = ( byte ) ( sr * byte.MaxValue / acount );
                            destinationpx.G = ( byte ) ( sg * byte.MaxValue / acount );
                            destinationpx.B = ( byte ) ( sb * byte.MaxValue / acount );
                        }
                        destination[x , y] = destinationpx;
                    }
                }
        ...
        public Task<PixelImage> RescaleAsync(int width , int height , ScalingMode mode = ScalingMode.Auto)
        {
            return Task.Run(() => Rescale(width , height , mode));
        }
        private void RescaleLinearFromSourceH(PixelImage destination , ScalingMode verticalMode)
        {
        ...
        private void RescaleLinearFromDestinationH(PixelImage destination , ScalingMode verticalMode)
        {
            var vmode = (ScalingMode)((int)verticalMode&0xFF00);
            double scaleX = (double)width / destination.width;
        ...
        private void RescaleV(PixelImage destination , ScalingMode verticalMode , int sx , int ex)
        {
            switch ( verticalMode )
            {
        ...
        private void RescaleLinearFromSourceV(PixelImage destination , int sx , int ex)
        {
        ...
        private void RescaleLinearFromDestinationV(PixelImage destination , int sx , int ex)
        {
            double scaleY = (double)height / destination.height;
        ...
        public PixelImage GetSubBitmap(Rectangle location)
        {
            PixelImage bmp = new PixelImage(location.Width,location.Height);
        ...
        public PixelImage GetBitmap(int x , int y , int w , int h)
        {
            return GetSubBitmap(new Rectangle(x , y , w , h));
        }
        public PixelImage Clone(Rectangle rectangle)
