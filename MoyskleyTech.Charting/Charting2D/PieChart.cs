...
namespace MoyskleyTech.Charting.Charting2D
{
    ...
    public partial class PieChart<R> : IDrawableChart<R>
        where R : unmanaged
    {
        ...
        public void Draw(Graphics<R> g, int bmpWidth, int bmpHeight)
        {
            ...
            {
                DrawDonut3D(g, width, height, totalWeight, ref angle, ref angleSpan, r);
            }
        ...
        private void DrawDonut3D(Graphics<R> g, int width, int height, double totalWeight, ref double angle, ref double angleSpan, int r)
        {
            ...
            angle = 2 * PI;
            enumColor = GetColors().GetEnumerator();
            //Draw top
            foreach (PieData pd in Datas)
            {
                enumColor.MoveNext();
